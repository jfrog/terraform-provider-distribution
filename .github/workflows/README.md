...
1. Never injects `JFROG_URL` / `JFROG_ACCESS_TOKEN` / Slack secrets
...
This provider does **not** spin up GKE. After the gate, the suite calls a shared Artifactory with `JFROG_URL` and `JFROG_ACCESS_TOKEN` (Go 1.21, no Helm). Treat those credentials the same way xray treats GKE/license secrets: only after approval.
...
After you approve, tests **do** run the customer SHA with Artifactory URL and access-token secrets. That is required to test their code. Treat the approval as a trust decision: do not approve a PR that looks like it will dump env or exfiltrate credentials.
...
A normal PR file adding a `CHANGELOG.md` bullet is unchanged. **Merging the PR lands that bullet.** CI does not write it.

### 2. CI “Tested on Artifactory …” heading (we cannot push to a fork)

The old job rewrote the version heading and `git push`ed to the PR branch. `GITHUB_TOKEN` cannot write to `customer/terraform-provider-distribution`.

| PR type | After tests pass |
|---|---|
| Same-repo (`jfrog/terraform-provider-distribution`) | CI still updates `CHANGELOG.md` and pushes, as before |
| Fork (customer) | Push is skipped. CI comments on the PR. Slack still asks for review. |

**While merging a customer PR:** resolve `CHANGELOG.md` if it conflicts (move their bullet under the current version), and add the “Tested on Artifactory …” suffix yourself if you want it. You can do that in the merge conflict UI, as a maintainer edit on their branch (if they allow edits), or as a small commit on `main` after merge.
...
These must be done on `jfrog/terraform-provider-distribution` after this lands on `main`:
...
| `acceptance-tests.yml` | `workflow_call` only | Artifactory + Terraform/OpenTofu tests + changelog |
| `cla.yml` | `pull_request_target` | CLA (unchanged) |
| `changelog.yml` | `pull_request` | CHANGELOG check (unchanged) |
